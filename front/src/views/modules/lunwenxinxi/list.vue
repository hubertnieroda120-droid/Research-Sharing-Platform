<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('lunwenxinxi','院审核') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="lunwenxiaoshenCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'院审核':'' }}</el-button>
                <el-button v-if="isAuth('lunwenxinxi','院审核') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="lunwenxiaoshenCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'院审核':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('lunwenxinxi','院审核') && contents.tableBtnIcon == 0" type="success" size="mini" @click="lunwenxiaoshenCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'院审核':'' }}</el-button>
    ...
    <lunwenxiaoshen-cross-add-or-update v-if="lunwenxiaoshenCrossAddOrUpdateFlag" :parent="this" ref="lunwenxiaoshenCrossaddOrUpdate"></lunwenxiaoshen-cross-add-or-update>
...
import lunwenxiaoshenCrossAddOrUpdate from "../lunwenxiaoshen/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      lunwenxiaoshenCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    lunwenxiaoshenCrossAddOrUpdate,
  },
  methods: {
    ...
    lunwenxiaoshenCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.lunwenxiaoshenCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.lunwenxiaoshenCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.lunwenxiaoshenCrossaddOrUpdate.init(row.id,type);
      });
    },
    chartDialog() {
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            url: "lunwenxinxi/group/lunwenleixing",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
            let res = data.data;
